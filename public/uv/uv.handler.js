(() => {
    var f = self.Ultraviolet, R = self.UVClient, I = self.__uv$config, c = self.__uv$bareData, d = self.__uv$bareURL, v = self.__uv$cookies;
    if (typeof c != "object" || typeof d != "string" || typeof v != "string") throw new TypeError("Unable to load global UV data");
    
    // --- Bot検知回避のためのパッチ (追加) ---
    const applyBotPatches = (win) => {
        try {
            // 1. webdriverを隠蔽
            Object.defineProperty(win.navigator, 'webdriver', { get: () => undefined });
            // 2. 自動操作ツールの痕跡を消去
            delete win.navigator.__webdriver_evaluate;
            delete win.navigator.__webdriver_unwrapped;
            // 3. 言語設定を一貫させる (サーバー側設定と同期)
            Object.defineProperty(win.navigator, 'languages', { get: () => ['ja-JP', 'ja', 'en-US', 'en'] });
            // 4. Chrome等の場合の不自然なプラグイン数を偽装
            if (win.navigator.plugins.length === 0) {
    ...
    function p(o) {
        ...
        // パッチを適用 (iframe内のウィンドウ等にも適用されるようにここに追加)
        applyBotPatches(o);

        o.document && o.window && o.document.querySelectorAll("script[__uv-script]").forEach(t => t.remove());
        let b = !o.window, _ = "__uv", s = "__uv$", e = new f(I), y = new f.BareClient(d, c), a = new R(o, y, b), {
            HTMLMediaElement: h,
            HTMLScriptElement: S,
            HTMLAudioElement: M,
            HTMLVideoElement: j,
        ...
        if (e.meta.url = e.location, e.domain = e.meta.url.host, e.blobUrls = new o.Map, e.referrer = "", e.cookies = [], e.localStorageObj = {}, e.sessionStorageObj = {}, e.location.href === "about:srcdoc" && (e.meta = o.parent.__uv.meta), o.EventTarget && (e.addEventListener = o.EventTarget.prototype.addEventListener, e.removeListener = o.EventTarget.prototype.removeListener, e.dispatchEvent = o.EventTarget.prototype.dispatchEvent), a.nativeMethods.defineProperty(a.storage.storeProto, "__uv$storageObj", {
            ...
            for (let t in o.localStorage) t.startsWith(s + e.location.origin + "@") && (e.localStorageObj[t.slice((s + e.location.origin + "@").length)] = o.localStorage.getItem(t));
            e.lsWrap = a.storage.emulate(a.storage.localStorage, e.localStorageObj)
        }
        if (o.sessionStorage) {
            for (let t in o.sessionStorage) t.startsWith(s + e.location.origin + "@") && (e.sessionStorageObj[t.slice((s + e.location.origin + "@").length)] = o.sessionStorage.getItem(t));
        ...
        a.nativeMethods.defineProperty(e.meta, "base", {
            get() {
                return o.document ? (a.node.baseURI.get.call(o.document) !== m && (m = a.node.baseURI.get.call(o.document), P = e.sourceUrl(m)), P) : e.meta.url.href
            }
        }), e.methods = {
            ...
            parent: s + "parent",
            top: s + "top"
        }, e.filterKeys = [_, e.methods.setSource, e.methods.source, e.methods.location, e.methods.function, e.methods.string, e.methods.eval, e.methods.parent, e.methods.top, s + "protocol", s + "storageObj", s + "url", s + "modifiedStyle", s + "config", s + "dispatched", "Ultraviolet", "__uvHook"], a.on("wrap", (t, r) => {
            ...
            e.cookie.db().then(l => {
                e.cookie.setCookies(t.data.value, l, e.meta), e.cookie.getCookies(l).then(i => {
            ...
            switch (t.that.tagName) {
                case "SCRIPT":
                    t.data.value = e.js.rewrite(t.data.value.replace(/window\.open\s*\(([^)]+)\)/g, 'self.location.href = $1'));
                    break;
                case "STYLE":
                    t.data.value = e.rewriteCSS(t.data.value);
                    break;
                default:
                    t.data.value = e.rewriteHtml(t.data.value.replace(/target\s*=\s*(["']?)[^"'\s>]*\1/gi, ''))
            }
            ...
            t.data.html = [e.rewriteHtml(t.data.html.join("").replace(/target\s*=\s*(["']?)[^"'\s>]*\1/gi, ''))]
        }), a.document.on("writeln", t => {
            if (!t.data.html.length) return !1;
            t.data.html = [e.rewriteHtml(t.data.html.join("").replace(/target\s*=\s*(["']?)[^"'\s>]*\1/gi, ''))]
        }), a.element.on("insertAdjacentHTML", t => {
            t.data.html = e.rewriteHtml(t.data.html.replace(/target\s*=\s*(["']?)[^"'\s>]*\1/gi, ''))
            ...
            if (t.data.name.toLowerCase() === "target") {
                t.respondWith(null);
                return
            }
            ...
            t.that && (l = t.that.__uv$source.call), t.data.origin = "*", t.data.message = {
                ...
                __to: r
            }, t.respondWith(b ? l(t.target, [t.data.message, t.data.transfer], t.that) : l(t.target, [t.data.message, t.data.origin, t.data.transfer], t.that))
        }), a.message.on("data", t => {
            let {
                value: r
            } = t.data;
            ...
            e.attrs.isUrl(t.data.name) && (t.target.call(t.that, e.attributePrefix + "-attr-" + t.data.name, t.data.value), t.data.value = e.rewriteUrl(t.data.value)), e.attrs.isStyle(t.data.name) && (t.target.call(t.that, e.attributePrefix + "-attr-" + t.data.name, t.data.value), t.data.value = e.rewriteCSS(t.data.value, {
                ...
                ...e.meta,
                document: !0,
                injectHead: e.createHtmlInject(e.handlerScript, e.bundleScript, e.clientScript, e.configScript, d, c, n, o.location.href)
            ...
            get: (t, r) => e.sourceUrl(t.call(r)),
            set: (t, r, [l]) => {
                if (new String(l).toString().trim().startsWith("blob:") && r instanceof h) return a.element.setAttribute.call(r, e.attributePrefix + "-attr-src", l), t.call(r, e.blobUrls.get(l) || l);
        ...
        }), a.element.hookProperty([L, O, C, T], "target", {
            get: (t, r) => null,
            set: (t, r, [l]) => {}
        });
        ...
        if (a.element.hookProperty(u, "contentWindow", {
                ...
                t.data.string = e.rewriteHtml(t.data.string.replace(/target\s*=\s*(["']?)[^"'\s>]*\1/gi, ''), {
                    ...e.meta,
                    document: !0
                })
                ...
                t.that.__uv$storageObj && (t.that.__uv$storageObj[t.data.name] = t.data.value), t.data.name = s + e.meta.url.origin + "@" + t.data.name
            }), a.storage.on("delete", t => {
                t.that.__uv$storageObj && delete t.that.__uv$storageObj[t.data.name], t.data.name = s + e.meta.url.origin + "@" + t.data.name
                ...
                if (t.that.__uv$storageObj)
                    for (let r of a.nativeMethods.keys.call(null, t.that.__uv$storageObj)) delete t.that.__uv$storageObj[r], a.storage.removeItem.call(t.that, s + e.meta.url.origin + "@" + r), t.respondWith()
                ...
                r.Origin = e.meta.url.origin, r["User-Agent"] = navigator.userAgent, n !== "" && (r.Cookie = n.toString()), t.respondWith(y.createWebSocket(t.data.args[0], t.data.args[1], {
                    headers: r,
                    readyStateHook: (l, i) => {
                ...
                t.data.script = e.rewriteJS(t.data.script.replace(/window\.open\s*\(([^)]+)\)/g, 'self.location.href = $1'))
            ...
            get: (t, r) => {
                ...
                if (!l[s + "modifiedStyle"])
                    for (let i of a.style.urlProps) a.nativeMethods.defineProperty(l, i, {
                        enumerable: !0,
                        configurable: !0,
                        get() {
        ...
        if (o.document) {
            o.document.addEventListener('DOMContentLoaded', () => {
                const links = o.document.querySelectorAll('a[href]');
                links.forEach(link => {
                    link.addEventListener('click', (event) => {
                        event.preventDefault();
                        self.location.href = e.rewriteUrl(link.href)
                    })
                });
                const forms = o.document.querySelectorAll('form');
                forms.forEach(form => {
                    form.addEventListener('submit', (event) => {
                        event.preventDefault();
                        const formData = new FormData(form);
                        let url = e.rewriteUrl(form.action);
                        if (form.method.toLowerCase() === 'get') {
                            ...
                            fetch(url, {
                                ...
                                if (response.redirected) {
                                    self.location.href = response.url
                                } else {
